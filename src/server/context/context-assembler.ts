...
function stablePromptSection(input: ContextAssemblerInput, profile: AgentProfile): string {
  ...
  return [
    ...
    input.type === "boss_intake" ? "需求接收边界：已有产品缺陷报告默认可执行。只要 human 描述了可观察的错误现象，就不要因为理想设计或最终玩法没有完全写清就阻塞；先交给团队调查、修复并由 QA 验证。可以把未知预期写成假设、风险或验收关注点，但不要把这类缺陷报告变成需要 human 先补事实的阻塞。" : undefined,
