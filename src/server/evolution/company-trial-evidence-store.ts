...
export class CompanyTrialEvidenceStore {
  constructor(private readonly homeDir: string, private readonly companyId: string, private readonly trials: CompanyTrialStore, private readonly proposals: ScopePromotionStore, private readonly now: () => Date = () => new Date(), private readonly observations: EvolutionObservationPort = EMPTY_EVOLUTION_OBSERVATION_PORT) {}
  ...
  async record(input: { commandId: string; trialId: string; targetWorkspaceRoot: string; observations: CompanyTrialObservation[]; startedAt: string; endedAt: string }): Promise<CompanyTrialEvidence> {
    ...
    return this.exclusive(async () => {
      ...
      await validateObservations(trial, input.targetWorkspaceRoot, observations, this.observations);
...
async function validateObservations(trial: Awaited<ReturnType<CompanyTrialStore["get"]>>, root: string, observations: CompanyTrialObservation[], telemetry: EvolutionObservationPort): Promise<void> {
  ...
  for (const item of observations) {
    ...
    if (!await telemetry.verifyRuntimeAssignment({ agentId: trial.target.agentId, traceId: item.traceRef.ref, promotionId: trial.trialId, releaseId: trial.trialReleaseRef.id, selected })) throw invalid("Company trial trace does not prove selected/control assignment");
    if (selected) {
      const proof = proofs.find((candidate) => candidate.proofId === item.inheritanceProofRef);
      if (!proof || proof.traceRef?.ref !== item.traceRef.ref || proof.traceRef.profileId !== trial.target.profileId || proof.releaseRef.id !== trial.trialReleaseRef.id || proof.releaseRef.contentHash !== trial.trialReleaseRef.contentHash) throw invalid("Selected company trial observation has no matching inheritance proof");
