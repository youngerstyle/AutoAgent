...
describe("TicketEngine single Plan flow", () => {
  ...
  it("runs shell-based assurance in a disposable worktree without integrating test side effects", async () => {
    const fixture = await createFixture({ gitIsolation: true });
    const planning = fixture.plan.graph.ticketIds[0]!;
    ...
    expect(await fixture.engine.applyTicket(ticketCommand(fixture.planId, assuranceId, claim!, "complete-shell-assurance", completePayload())))
      .toMatchObject({ accepted: true, ticketStatus: "completed", planStatus: "completed" });
