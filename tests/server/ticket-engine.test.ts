...
describe("TicketEngine single Plan flow", () => {
  ...
  it("rejects dirty shell-based assurance and retains its disposable worktree for cleanup", async () => {
    const fixture = await createFixture({ gitIsolation: true });
    const planning = fixture.plan.graph.ticketIds[0]!;
    const planningClaim = await fixture.engine.claimReady({ requestId: "claim-assurance-planning", planId: fixture.planId, ticketId: planning, expectedTicketVersion: 1, principalId: "planner", leaseDurationMs: 60_000 });
    await fixture.engine.applyPlan({
      commandId: "append-shell-assurance", planId: fixture.planId, actorPrincipalId: "planner", issuedAt: now,
      payload: {
        type: "apply_change", expectedPlanVersion: 2, sourceTicketId: planning,
        sourceAuthority: { kind: "claim", claimId: planningClaim!.claimId, fencingToken: planningClaim!.fencingToken },
        change: {
          additions: [{
            ...draft("shell-assurance", "隔离验收"),
            assignment: { principalId: "qa", requiredTools: ["readFile", "shell"] },
            outputContract: { schemaRef: "mission-assurance-v1" },
          }],
          dependencyAdditions: [{ from: { ticketId: planning }, to: { clientRef: "shell-assurance" } }],
          cancelTicketIds: [], requiredTerminalRefs: [{ clientRef: "shell-assurance" }],
        },
      },
    });
    await fixture.engine.applyTicket(ticketCommand(fixture.planId, planning, planningClaim!, "complete-assurance-planning", completePayload()));
    const assuranceId = (await fixture.engine.getPlan(fixture.planId)).graph.ticketIds[1]!;
    const claim = await fixture.engine.claimReady({ requestId: "claim-shell-assurance", planId: fixture.planId, ticketId: assuranceId, expectedTicketVersion: 2, principalId: "qa", leaseDurationMs: 60_000 });
    const running = await fixture.engine.getTicket(assuranceId);
    const attempt = running!.attempts.find((candidate) => candidate.attemptId === claim!.attemptId)!;
    const isolatedRoot = attempt.workspaceBaseline!.isolation!.rootPath;
    await writeFile(path.join(isolatedRoot, "test-output.txt"), "generated by verification\n", "utf8");
    await writeFile(path.join(isolatedRoot, ".gitignore"), ".autoagent/\ndist-cache/\n", "utf8");

    expect(await fixture.engine.applyTicket(ticketCommand(fixture.planId, assuranceId, claim!, "complete-dirty-shell-assurance", completePayload())))
      .toMatchObject({
        accepted: false,
        code: "workspace_changes_not_allowed",
        reason: expect.stringContaining("test-output.txt"),
      });
    expect(await fixture.engine.applyTicket(ticketCommand(fixture.planId, assuranceId, claim!, "repeat-dirty-shell-assurance", completePayload())))
      .toMatchObject({ accepted: false, code: "workspace_changes_not_allowed", reason: expect.stringContaining(".gitignore") });
    expect(await fixture.engine.getTicket(assuranceId)).toMatchObject({ status: "running" });
    expect(existsSync(isolatedRoot)).toBe(true);
    expect(existsSync(path.join(fixture.root, "test-output.txt"))).toBe(false);

    await rm(path.join(isolatedRoot, "test-output.txt"));
    await git(isolatedRoot, ["restore", "--", ".gitignore"]);
    expect(await fixture.engine.applyTicket(ticketCommand(fixture.planId, assuranceId, claim!, "complete-clean-shell-assurance", completePayload())))
      .toMatchObject({ accepted: true, ticketStatus: "completed", planStatus: "completed" });
    expect(existsSync(isolatedRoot)).toBe(false);
    expect((await fixture.engine.getTicket(assuranceId))?.attempts[0]?.changeSet?.integration).toBeUndefined();
  });
