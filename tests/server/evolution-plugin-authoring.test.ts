...
describe("Provider-authored local Plugin evolution", () => {
  it("turns a tool-level Practice into a scanned critical-risk Candidate without activating it", async () => {
    ...
    const bindings = new PracticeBindingStore(root, now); const candidates = new EvolutionStore("workspace-a", root, now);
    ...
    const result = await new PluginAuthoringWorker("workspace-a", root, new FixtureAuthor(), now).run();
