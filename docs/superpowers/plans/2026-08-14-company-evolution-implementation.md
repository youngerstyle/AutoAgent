...
- Milestone E–G：Plugin/Harness Evolution 已按独立规范实现。包括内容寻址 Bundle、逐文件 provenance、静态供应链 Scanner、OS Sandbox Launcher 强制 gate、Node Permission Model 纵深防御、最小环境、内存/时间/协议限额、只读 capability broker、Plugin 工具贡献、Harness pre/post tool guardrail、stable canary mount、production mount 和 pointer fingerprint rollback unload。可执行扩展始终为 critical risk，Coordinator 不自动晋升，canary/production 只接受 human 批准；未配置受信 Launcher 时 fail closed。
