...
- 自研上下文压缩执行路径。
- 自研的模型驱动工具循环。工作区文件、命令和后台进程仍由 Host
  工具层实现和授权，Pi 只按原生 tool-call 协议调用它们。
...
- `shell` 启动的短命令可同步返回；超过短等待窗口仍在运行的进程必须
  立即返回稳定的进程句柄，并由 `pollProcess` 查询状态和日志。长驻服务
  不能占住一个 Agent Turn 等待退出。无共享状态的工具可以并行执行；
  共享同一浏览器会话、进程句柄或其他有序状态的调用必须由 Tool Runtime
  按到达顺序串行化，不能把并发控制责任交给模型或底层 CLI。
- `goal_resolution` 是 Host 自定义工具，不属于项目文件工具，也不能由 Pi 自动解释为工单完成。
- `startService` 的成功证据必须归属于本次启动的受管进程。同一端口的并发启动必须原子串行；端口可访问但本次进程已经退出时必须失败，不能把另一个 Agent 或旧版本服务的页面认领为当前交付证据。
...
- 需求接收 Ticket 正式提交的对齐目标 handoff 是所有下游 Goal、Ticket 成功标准和最终验收的语义上界；human 原始诉求只用于需求接收和审计。
- Agent 可以用可逆默认值补充已对齐目标中没有说明的实现细节，但不能用“首版”“MVP”“核心版”等新口径削弱明确限定词。
- 如果已对齐目标不可行、成本异常或必须缩小范围，Agent 应提交风险、阻塞或计划变更提案；未经正式变更，不能把降级交付判为目标完成。
...
- Agent 的文件读取权限和 Ticket 的交付归属是两件事。开发策略可以允许读取本机资料，但读取到的外部文件不会因此成为当前项目的交付物。
- 文件工具的相对路径永远属于当前 Workspace；`..` 不能改变工具作用域。开启“访问本机”后，外部访问仍必须使用显式绝对路径。
- `goal_resolution` 中声明为 `tool`、`file`、`document` 或 `artifact` 的文件证据必须位于当前 Workspace 根目录内。
- 外部项目、历史项目和兄弟目录中的文件可以作为参考输入，但不能用于证明当前 Ticket 已完成。
- Host 只校验证据归属与协议完整性，不用业务关键字判断功能是否完成；交付质量仍由下游独立验证 Ticket 判断。
